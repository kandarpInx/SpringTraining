              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <%
                	try(Session s=HibernateUtil.getSessionFactory().openSession())
                	{
                		...
                		List<CompanyPost> li=s.createQuery("from CompanyPost cp where cp.companyRegistration.id=:company_id order by cp.post_date desc", CompanyPost.class)
                								.setParameter("company_id", company_id)
                								.list();
                		
                		for(CompanyPost m:li)
                		{
                			%>
                			<tr>
			                  <td><%= m.getId() %></td>
			                  <td><%= m.getPost_date() %></td>
			                  <td><%= m.getPost() %></td>                  			                  
			                </tr>
                			<%
                		}
